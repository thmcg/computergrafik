...
void Simulation::loop(const double time, UpdateModelCallback updateModelCallback)
{
    // A=65; S=83; D=68; W=87
    ...
    for (auto &[i, scene] : scenes)
    {
        scene.update(time, scripts, updateModelCallback);
    }
}
...
size_t Simulation::loadScene(const std::string &filename, LoadModelCallback loadModelCallback)
{
    size_t sceneId = ++currentSceneId;
    scenes.emplace(
        std::piecewise_construct,
        std::forward_as_tuple(sceneId),
        std::forward_as_tuple(filename, loadModelCallback)
    );
    return sceneId;
}

void Simulation::unloadScene(size_t sceneId, UnloadModelCallback unloadModel)
{
    if (auto it = scenes.find(sceneId); it != scenes.end())
    {
        it->second.unload(unloadModel);
        scenes.erase(it);
    }
}

void Simulation::registerScript(const std::string &name, ScriptCallback script)
{
